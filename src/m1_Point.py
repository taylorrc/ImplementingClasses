...
class Point(object):
    """ Represents 2D point in x-y space. """

    def __init__(self, x, y):
        """ Constructs a new Point located at given x y. """
        self.start_x = x
        self.start_y = y
    ...
    def __repr__(self):
        """ Returns a string that represents a Point like 'Point (x,y)' """
        return 'Point (' + str(self.x) + ',' + str(self.y) + ')'

    def clone(self):
        """ Creates a new Point object with the same coordinates as self"""
    ...
    def get_distance_from_start(self):
       ...
       to the position that the Point was at when it was constructed."""
        ...
        start_distance = self.get_distance_from(start)

        return start_distance

    def get_distance_traveled(self):
        """Returns the sum of all the distances that
       this Point has "moved" via calls to   move_to  and/or   move_by."""
        return self.total_distance

    def closer_to(self, p2, p3):
        """Returns whichever of p2 and p3 this Point is closer to."""
        p2_distance = self.get_distance_from(p2)
        p3_distance = self.get_distance_from(p3)
    ...
    def halfway_to(self, p2):
        ...
        new_point = Point((p2.x + self.x)/2, (p2.y + self.y)/2)
        return new_point
